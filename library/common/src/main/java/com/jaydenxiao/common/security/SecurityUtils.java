...
import com.jaydenxiao.common.BASE64Decoder.encoder.BASE64Encoder;
...
public class SecurityUtils {
     ...
     */
    public static String hmacSha1(String base, String key) throws NoSuchAlgorithmException, InvalidKeyException, UnsupportedEncodingException {
        if (TextUtils.isEmpty(base) || TextUtils.isEmpty(key)) {
            return "";
        }
        //对整体源串进行一次url编码
        String encodebase = URLEncoder.encode(base,"UTF-8");
        String type = "HmacSHA1";
        SecretKeySpec secret = new SecretKeySpec(key.getBytes(), type);
        Mac mac = Mac.getInstance(type);
        mac.init(secret);
        //使用编码后的源串进行SHA1加密
        byte[] digest = mac.doFinal(encodebase.getBytes());
        String hmacSha1 = new BASE64Encoder().encode(digest);
//        LogUtils.loge("第一次编码sha1值" + hmacSha1);

        return Base64.encodeToString(hmacSha1.getBytes(), Base64.NO_WRAP);
